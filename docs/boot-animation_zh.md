...
每帧只更新中央区域；署名在首帧和淡入时绘制，独立定位，不受中央画布长宽比影响。
绘制缓冲区重复使用；首条不透明指令覆盖整个更新区域时，不再从 framebuffer 回读旧像素。
...
stage-2 在原进程仍绘制时预载并校验全部素材，然后等待原进程退出并取得同一文件锁。
交接保留最后一帧和 graphics VT，不清空整屏；两个阶段使用同一单调时钟选择循环帧，
负载高时跳过迟到帧，避免循环整体变慢。新素材损坏时不先停掉仍可用的旧动画。
stage-2 服务核对首帧就绪文件中的进程 PID 后才允许显示管理器启动，避免把旧进程的
就绪标记误认为新进程已启动。显示管理器接管前等待绘制停止确认。
...
旧 initrd 未写入决定时，generator 也会保存一次性判断；`daemon-reload` 不会在重启标记
清理后重新分类。手动世代选择的强制重启同样写入正常启动标记，有效的待启动世代优先按正常启动处理。

充电监控与动画共享同一 POSIX 文件锁和 VT2；充电服务与桌面、开机动画、故障控制台
相互排斥并有停止顺序。切换到诊断控制台时先禁止新动画、等待当前绘制退出，再显示日志。
上游 `Splash.kill()` 属于 stage-1 故障路径，必须保留诊断出口，不应当作正常 switch_root 交接屏蔽。
